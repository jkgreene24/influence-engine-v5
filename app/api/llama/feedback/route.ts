import { LlamaCloudIndex, SimilarityPostprocessor, } from "llamaindex";
...
export async function POST(request: Request) {
    const { query, userInfluenceStyle } = await request.json();
    const retriever = index.asRetriever({
        ...
        filters: {
            filters: [
                {
                    key: "userInfluenceStyle",
                    operator: "==",
                    value: userInfluenceStyle,
                },
            ]
        }
    });
